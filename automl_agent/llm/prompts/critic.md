...
Facts about this dataset that no metric in this prompt shows — from the profiler's own checks and
...
5. Check `dropped_hyperparams` in the result before you prescribe. A key listed there
   ...
   `What the executor can and cannot do` says the executor will do.
...
`specificity`, `balanced_accuracy_cut_headroom` and `balanced_accuracy_at_best_cut` are not in
...
7. Before you reach for the operating point at all, read `balanced_accuracy_cut_headroom`. It is
   `balanced_accuracy_at_best_cut` minus the `balanced_accuracy` the attempt achieved —
   the exact amount that a better decision threshold is still worth on *these* rows. When it
   is small the cut is already near-optimal and no lever on that axis has anything left to
   ...
   as a score the attempt reached — it is measured at a cut this attempt did not use.
   When it is large, there are two levers on that axis and they are alternatives, not a
   pair. `tune_threshold: true` moves the cut directly, in one attempt, and costs 20% of the
   training rows (reported as `cut_held_out_rows`); the imbalance weight moves the fit, so it
   moves the ranking too and reaches the cut only indirectly. Prescribe one. If the attempt
   already carries `applied_threshold`, the cut was chosen on held-back rows and the
   `balanced_accuracy_cut_headroom` you are reading is what that choice failed to transfer — which is evidence
   about the ranking or about those rows, not an argument for choosing again.
...
9. A difference the measurement cannot resolve is not evidence — and which measurement
   ...
   only) `recall` against `specificity` and `balanced_accuracy_cut_headroom`.
...
9b. `applied_pipeline` is what the preprocessing really was, when the attempt declared one:
    one line per step, in order, with the columns each touched. Read it rather than the plan's
    `pipeline`, for the reason `applied_hyperparams` outranks the proposal — a step naming a
    column the file does not have is dropped, and `(auto)` marks one the executor added because
    the family needed it. An attempt whose spec lost a step is not evidence about that step.
