...
**`--metric`** — 기본값 `f1`은 분류의 무난한 출발점이라는 뜻일 뿐, 이 데이터셋에 대한 주장이
아니다. 카드를 이미 읽었으니 네가 후보를 좁혀서 제시하고, 고르는 것은 사람에게 맡겨라. 물을
것은 지표 이름이 아니다 — **"이 모델이 틀릴 때 어느 쪽 틀림이 더 비싼가"**다.

- 다수 클래스 비율(chance)이 `accuracy`의 고정 기본 바 0.85보다 높으면 `accuracy`를 **후보에서
  빼라.** 다수 클래스만 찍는 모델이 목표를 달성한다 — 이 프로젝트의 임상 코호트에서 실제로
  그랬다 (chance 0.8899)
- 양성이 드물면 `pr_auc`가 `roc_auc`보다 민감하다. 대신 `pr_auc`의 chance는 0.5가 아니라 **양성
  비율**이므로, 0.3이 나쁜 점수인지는 카드를 봐야 안다
- 놓치는 쪽이 비싸면(질환을 못 잡는 것) `recall`, 잘못 부르는 쪽이 비싸면 `precision`. 한쪽만
  보는 목표이므로 **다른 쪽은 `result.json`에서 함께 읽는다고 말해 줘라**
- 불균형에서 둘을 함께 보고 싶으면 `balanced_accuracy`. 이 프로젝트에서 가장 큰 레버
  (`tune_threshold`)가 측정된 지표가 이것이다
- 회귀는 `r2`(단위 없음, 1이 상한) / `mae`(정답 열의 단위 그대로) / `rmse`(큰 오차에 더 민감).
  **`mae`·`rmse`는 지표별 기본 바가 없다** — 카드에 기준선이 없으면 `--threshold`를 줘야 실행이
  성립한다

task는 묻지 마라. 정답 열이 정한다. 연속 열에 분류 지표를 주면 거부가 아니라 그 task의 기본
지표로 **바뀌어 실행되고**, 바꿨다는 사실만 한 줄로 남는다.
...
- `--metric`은 2단계에서 사람이 고른 것을 그대로 쓴다. 생략해서 기본값으로 넘기지 마라 — 어느
