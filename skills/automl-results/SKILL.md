...
벗어나는 것이 관측됐다. Δ 하나는 처치의 증거로 부족하다.
...
인용하기 전에 알아야 하는 핵심 판정 네 개:

- `llm` 팔이 **random search**를 이긴다 — 이진 4개 중 3개, 사전 등록 기준 충족
- `llm` 팔이 **`no_llm`(규칙 기반)**을 이기는지는 **구분되지 않는다** — 사전 등록 미달
- 측정된 개선 둘은 **코드 레버**다 — 결정 컷 +0.0762, 정렬된 spec +0.0046. LLM의 기여로 돌리지 마라
- 시드가 하나이고 시드 잡음이 평균 0.1472다

**넷 다 이 저장소에서 검산할 수 없다.** 그러니 숫자를 말할 때 출처가 개발 저장소라는 것을
같이 말해라 — 여기서 돈 실행의 결과와 섞으면 안 된다.
