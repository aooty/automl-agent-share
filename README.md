...
쓰였는지는 아티팩트의 `structured_mode`에 남습니다. ollama 경로는 그 둘 중 어느 것도
아니라 `format`으로 기록됩니다. 어느 전송으로 갔는지는 같은 아티팩트의 `route`가 말하고,
그것은 호출한 클라이언트의 것이므로 제안자와 심판이 다른 전송이면 값도 다릅니다.
